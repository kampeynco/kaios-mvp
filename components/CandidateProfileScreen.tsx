...
import { User, Flag, Swords, Plus, Save, Palette, Upload, Trash2, Pen, Check, Copy, X, Bold, Italic, AlertTriangle, Download, CloudUpload, Search, ChevronDown } from 'lucide-react';
    ...
    <div className="flex h-full bg-gray-50/50 dark:bg-background-dark transition-colors duration-300">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-8">
          ...
          {activeTab === 'opponents' && <OpponentsView />}
...
const BioView: React.FC = () => {
  const [coreValues, setCoreValues] = useState(['Community First', 'Transparency', 'Innovation', 'Equity']);
      ...
      <div className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-subtle space-y-6 transition-colors">
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input type="text" defaultValue="Sarah Jenkins" className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-black/5 dark:focus:ring-white/10 focus:border-black dark:focus:border-white text-text-main dark:text-white outline-none transition-all" />
          </div>
          <div>
            <label className="block text-sm font-medium text-text-main dark:text-white mb-2">Current Title/Role</label>
            <input type="text" defaultValue="Community Organizer" className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-black/5 dark:focus:ring-white/10 focus:border-black dark:focus:border-white text-text-main dark:text-white outline-none transition-all" />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-text-main dark:text-white mb-2">Short Biography</label>
          <textarea rows={4} className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-black/5 dark:focus:ring-white/10 focus:border-black dark:focus:border-white text-text-main dark:text-white outline-none transition-all resize-none" defaultValue="Sarah Jenkins represents a new generation of leadership..." />
...
const PlatformView: React.FC = () => {
  const [issues, setIssues] = useState<Issue[]>([
    { id: '1', title: "Affordable Healthcare", status: "Published", description: "Expanding access to community health centers and reducing prescription costs." },
    { id: '2', title: "Green Infrastructure", status: "Draft", description: "Investing in renewable energy projects to create local jobs." },
    { id: '3', title: "Education Reform", status: "Published", description: "Increasing funding for public schools and teacher salaries." }
  ]);
          ...
          <div key={issue.id} className="bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-800 shadow-subtle hover:shadow-md transition-all cursor-pointer group relative">
            <div className="flex justify-between items-start mb-2 pr-8">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                issue.status === 'Published' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 
...
const OpponentsView: React.FC = () => {
  const [opponents, setOpponents] = useState<Opponent[]>([
    { id: '1', name: 'John Doe (Incumbent)', party: 'Opposition Party', tags: ['Voted Against HR. 405', 'Absentee Record'] }
  ]);
  const [isAddOpen, setIsAddOpen] = useState(false);
  const [deleteId, setDeleteId] = useState<string | null>(null);
...
const BrandKitView: React.FC = () => {
  ...
  const [photos, setPhotos] = useState<BrandAsset[]>([
  ...
  const handleLogoUpload = (details: { acceptedFiles: File[] }) => {
    const newLogos = details.acceptedFiles.map((file, idx) => ({
      id: `up-logo-${Date.now()}-${idx}`,
  ...
  const handlePhotoUpload = (details: { acceptedFiles: File[] }) => {
    const newPhotos = details.acceptedFiles.map((file, idx) => ({
      id: `up-photo-${Date.now()}-${idx}`,
      type: 'upload' as const,
      url: URL.createObjectURL(file),
      name: file.name
    }));
