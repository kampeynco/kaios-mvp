...
import { Folder, Plus, MoreHorizontal, Pencil, Trash2, AlertTriangle, Loader2 } from 'lucide-react';
...
export const ProjectsScreen: React.FC<ProjectsScreenProps> = ({
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      setProjects(data);
      if (data.length > 0) {
        // Keep active project if it still exists, otherwise pick first
  ...
  const handleCreateProject = async (name: string, memory: 'default' | 'project-only') => {
    if (!workspaceId) return;
    try {
  ...
  const handleRenameSubmit = async (newName: string) => {
    ...
    try {
      const updated = await projectService.updateProject(renamingProject.id, { name: newName });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await projectService.deleteProject(deleteConfirmation.id);
      setProjects(prev => prev.filter(p => p.id !== deleteConfirmation.id));
      if (activeProject?.id === deleteConfirmation.id) {
        setActiveProject(projects.find(p => p.id !== deleteConfirmation.id) || null);
      }
  ...
  // Mock data for files (remains for UI consistency until draftService is used)
  const projectFiles = [
    {
      ...
      <aside className="w-64 bg-white dark:bg-background-dark border-r border-gray-200 dark:border-gray-800 flex-shrink-0 flex flex-col transition-colors duration-300">
        ...
        <div className="p-4 flex-1 overflow-y-auto">
          <button
            ...
            className="w-full flex items-center gap-2 justify-center px-4 py-2 bg-black dark:bg-white text-white dark:text-black text-sm font-medium rounded-lg hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors mb-4 focus:outline-none"
          >
          ...
          <nav className="space-y-1">
            {projects.map((project) => (
              <div
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 text-sm font-medium rounded-lg transition-colors group cursor-pointer ${activeProject?.id === project.id
              ...
              >
                <div className="flex items-center gap-3 truncate pointer-events-none">
                  <Folder className={`w-4 h-4 ${activeProject?.id === project.id ? 'text-black dark:text-white' : 'text-gray-400'}`} />
                ...
                <div className="relative">
                  <button
                    ...
                    className={`p-1 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors ${dropdownOpen === project.id
      ...
      <div className="flex-1 overflow-y-auto bg-white dark:bg-background-dark">
        <div className="max-w-4xl mx-auto px-8 py-12">
          {activeProject ? (
            <div className="animate-in fade-in duration-300">
              ...
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-3xl font-display font-normal text-text-main dark:text-white tracking-tight">{activeProject.name}</h1>
                </div>
              ...
              <div className="w-full bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-[0_2px_8px_rgba(0,0,0,0.04)] overflow-hidden transition-all focus-within:shadow-[0_4px_12px_rgba(0,0,0,0.08)] focus-within:border-gray-300 dark:focus-within:border-gray-700 group mb-12">
                <div className="p-1">
                  ...
                  <textarea
                    ...
                    placeholder={`Start a new chat in ${activeProject.name}...`}
                    value={prompt}
            ...
            <div className="flex flex-col items-center justify-center h-[60vh] text-text-sub dark:text-gray-400">
